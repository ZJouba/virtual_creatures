...
def modify_doc(doc):
    ...
    x_points = [[[[1, 2, 2, 1], [1.2, 1.6, 1.6], [1.8, 1.8, 1.6]]]]
    y_points = [[[[3, 3, 4, 4], [3.2, 3.6, 3.2], [3.4, 3.8, 3.8]]]]
    polygon = ColumnDataSource(data=dict(x=x_points, y=y_points))
    ...
    def plot_creature(event):
        if len(source.selected.indices) > 0:
            ...
            p3.x_range = (np.argmin(coords), np.argmax(coords))
